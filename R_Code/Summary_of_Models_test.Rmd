...
age <- bSpline(day,Boundary.knots =c(min(data$day),max(data$day)) , knots = c(-4,-2,3),degree = 3)
...
```
## splines a for day and age ai_spline_age_day
### Summary for day and age
```{r }
plot(ai_spline_age_day$samples)
effectiveSize(ai_spline_age_day$samples) 
#Tine-series SE is monte carlo standard error var von dem ding durch samplsize
summary(ai_spline_age_day$samples)
ai_spline_age_day$WAIC$WAIC
geweke.diag(ai_spline_age_day$samples)
geweke.plot(ai_spline_age_day$samples)
gelman.diag(ai_spline_age_day$samples, confidence = 0.95)
gelman.plot(ai_spline_age_day$samples, confidence = 0.95)
autocorr.diag(ai_spline_age_day$samples)

```


### with splines a for day and age a plot
```{r}
source("helpfunctions.r")
#beta 1 age beta2 day
...
legend("topright", legend = c("SS", "HB"),lwd = 3,
               ...
               title = "Groups")
```
## splines a for day and age ai_spline_day
### Summary for day and age
```{r }
plot(ai_spline_age_day$samples)
effectiveSize(ai_spline_age_day$samples) 
