...
model.inits <- list(
  list(beta0 = 0,   beta_age=0, beta_day = 0,    phi = 5, sigma_b0 = 1.0, sigma_b1 = 1,b0 = rep(0.0,times = subjects), b1 = rep(0.0,times = subjects)),
  list(beta0 = 0.5, beta_age=1, beta_day = -0.1, phi = 10, sigma_b0 = 1.5, sigma_b1 = 1,b0 = rep(0.2,times = subjects), b1 = rep(0.0,times = subjects)),
  list(beta0 = 1,   beta_age=1, beta_day = 0.1,  phi = 7, sigma_b0 = 0.5, sigma_b1 = 1,b0 = rep(0.3,times = subjects), b1 = rep(0.0,times = subjects))
)
parameters = c("beta0", "beta_age", "beta_day","sigma_b0","sigma_b1","phi","Deviance","ppo","b0","b1")

model.constants <- list( N = length(data1$sofa), x1 = data1$age,
                         id = data1$id, x2 = data1$day,  Nsubj = length(unique(data1$id)))
model.data <- list(y = data1$ai, y_pred = rep(NA,length(data1$ai)))
...
beta_rand <- nimbleMCMC(
  ...
  monitors = parameters, nchains = 3, niter = 9000,
  nburnin = 4000, thin = 10, setSeed = c(1,2,3),
  samplesAsCodaMCMC = T, samples = T, # get an coda object instead of plain values
  WAIC = T ) # get the WAIC

saveRDS(beta_rand, file = "..\\data\\mcmc_res\\beta_rand_rslope.rds")
beta_rand$WAIC$WAIC
