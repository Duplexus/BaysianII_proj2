...
parameters = c("beta0", "beta_age", "beta_day","sigma_b0","sigma_b1","sigma_b2","sigma_b3","phi","Deviance","ppo","b0")
...
model.function <- nimbleCode({
  for (i in 1:N){
    ...
    logit(mu[i]) <- beta0 + beta_age *x1[i] + beta_day *x2[i]+ b0[id[i]]+ 
      x2[i] *b1[id[i]]+ pow(x2[i],2) *b2[id[i]]+ pow(x2[i],3) *b3[id[i]]
...
saveRDS(beta_rand, file = "..\\data\\mcmc_res\\beta_rand_rslope_trip.rds")
